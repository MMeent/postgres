...
#include "utils/guc_collection.h"
...
static bool call_real_check_hook(const struct config_generic *conf, double *newval,
								 void **extra, GucSource source, int elevel);
static bool call_string_check_hook(const struct config_generic *conf, const char **newval,
								   void **extra, GucSource source, int elevel);
 ...
 */
static bool
string_field_used(struct config_generic *conf, const char *strval)
{
 ...
 */
static void
set_string_field(struct config_generic *conf, const char **field, const char *newval)
{
	const char *oldval = *field;
	...
	*field = newval;
	...
	if (oldval && !string_field_used(conf, oldval))
		guc_free(unconstify(char *, oldval));
}
...
add_placeholder_variable(const char *name, int elevel)
{
	 ...
	 */
	var->_string.variable = (const char **) (var + 1);
			...
			if (restorePrior || restoreMasked)
			{
				...
				switch (gconf->vartype)
				{
					...
					case PGC_STRING:
						{
							struct config_string *conf = &gconf->_string;
							const char *newval = newvalue.val.stringval;
							void	   *newextra = newvalue.extra;
		...
		case PGC_STRING:
			{
				...
				{
					char *copy = unconstify(char *, newval->stringval);

					truncate_identifier(copy,
										strlen(copy),
										true);
				}
				...
				if (!call_string_check_hook(record, &newval->stringval, newextra,
											source, elevel))
				{
					guc_free(unconstify(char *, newval->stringval));
			...
			case PGC_STRING:
				{
					...
					guc_free(unconstify(char *, *conf->variable));
					if (conf->reset_val && conf->reset_val != *conf->variable)
						guc_free(unconstify(char *, conf->reset_val));
					break;
				}
		}
...
static bool
call_string_check_hook(const struct config_generic *conf, const char **newval, void **extra,
					   GucSource source, int elevel)
{
